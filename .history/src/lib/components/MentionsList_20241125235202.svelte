<script lang="ts">
  ...
  async function handleGenerateReply(tweet: TweetV2) {
    try {
        ...
        // Save the response first to get the generated ID
        const savedResponse = await DatabaseService.saveResponse({
            id: ID.unique(), // This will be ignored now
            tweet_id: tweet.id,
            content: reply,
        ...
        // Create response object using the ID from saved document
        const newResponse = {
            id: savedResponse.$id, // Use the Appwrite-generated ID
            content: reply,
            generated_at: timestamp,
            model: 'gemini-1.5-flash'
        };
