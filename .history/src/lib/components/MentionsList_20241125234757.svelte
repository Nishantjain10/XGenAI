<script lang="ts">
  ...
  async function handleGenerateReply(tweet: TweetV2) {
    try {
        ...
        if (!tweetExists) {
            // Only save tweet if it doesn't exist
        ...
        // Create a unique response object using Appwrite's ID.unique()
        ...
        try {
            ...
            const savedResponse = await DatabaseService.saveResponse({
                ...
                tweet_id: tweet.id,
                content: reply,
                generated_at: newResponse.generated_at,
                model: 'gemini-1.5-flash',
                posted: false
            });

            // Only update the UI if save was successful
            if (savedResponse) {
                generatedReplies[tweet.id] = [
                    {
                        id: savedResponse.$id,
                        content: savedResponse.content,
                        generated_at: savedResponse.generated_at,
                        model: savedResponse.model
                    },
                    ...generatedReplies[tweet.id]
                ];
            }
