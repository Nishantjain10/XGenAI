...
export async function GET({ request }) {
    try {
        ...
        // Update URL to fetch only the most recent mention
        const url = new URL('https://api.twitter.com/2/users/me/mentions');
        url.searchParams.append('max_results', '1'); // Changed from 20 to 1
        ...
        if (mentionsResponse.status === 429) {
            ...
            const mockData = generateMockMentions(5); // Keep mock data same
